...
# Flask Setup
#################################################
app = Flask(__name__,static_url_path='/Images/surfsup.png')
...
def welcome():
    ...
    return (
        ...
        f"When given the start date (YYYY-MM-DD), calculates the minimum, average, and maximum temperature for all dates greater than and equal to the start date: /api/v1.0/start<br/>"
        f"When given the start and the end date (YYYY-MM-DD), calculate the minimum, average, and maximum temperature for dates between the start and end date: /api/v1.0/start/end<br/>"
...
def tobs():
    ...
    tobs_from_last_year = session.query(Measurement.date, Measurement.tobs).filter(
        Measurement.station == most_active_station_id).filter(Measurement.date >= query_start_date ).all()

    session.close()

    # Convert the query results to a dictionary using date as the key and temperature as the value.
    all_temperatures = []
    for date, temp in tobs_from_last_year:
        if temp != None:
            temp_dict = {}
            temp_dict[date] = temp
...
def determine_temps_for_date_range(start, end):
    ...
    if no_temperature_data == True:
        return f"No temperature data found for the introduce date range. Please try another date range."
    else:
